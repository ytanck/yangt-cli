...
const { getRepoList, getRepoTags } = require("./api");
...
class Creator {
  ...
  async getRepoInfo() {
    ...
    loading.start();
    const { data: repoList } = await getRepoList();
    loading.succeed();
  ...
  async getTagInfo(repo) {
    ...
    loading.start();
    const res = await getRepoTags(repo);
    loading.succeed();
    if (!res) return;
  ...
  async getCollectRepo() {
    ...
    loading.start();
    try {
      const { data: list } = await getRepoList({ per_page: 100 }); //仓库信息
      // const { data: list } = await getRepoTags('仓库名');//tags信息
      loading.succeed();
      const reposTemplate = list.map((item) => item.name);
